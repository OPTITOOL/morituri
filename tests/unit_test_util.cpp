...
TEST_CASE("Shapefile exists", "[SHP exist]") {
  CHECK(shp_file_exists("tests/testdata/faroe-islands-latest/roads.shp") ==
        true);
  ...
  CHECK(shp_file_exists(std::string(
            "tests/testdata/faroe-islands-latest/roads.shp")) == true);
  CHECK(shp_file_exists(std::string(
            "tests/testdata/faroe-islands-latest/README")) == false);
  CHECK(shp_file_exists(
            std::string("tests/testdata/faroe-islands-latest/???")) == false);

  CHECK(shp_file_exists(boost::filesystem::path(
            "tests/testdata/faroe-islands-latest/roads.shp")) == true);
  CHECK(shp_file_exists(boost::filesystem::path(
            "tests/testdata/faroe-islands-latest/README")) == false);
  CHECK(shp_file_exists(boost::filesystem::path(
            "tests/testdata/faroe-islands-latest/???")) == false);
}

TEST_CASE("DBF exists", "[DBF exist]") {
  CHECK(dbf_file_exists("tests/testdata/faroe-islands-latest/roads.dbf") ==
        true);
  ...
  CHECK(dbf_file_exists(std::string(
            "tests/testdata/faroe-islands-latest/roads.dbf")) == true);
  CHECK(dbf_file_exists(std::string(
            "tests/testdata/faroe-islands-latest/README")) == false);
  CHECK(dbf_file_exists(
            std::string("tests/testdata/faroe-islands-latest/???")) == false);

  CHECK(dbf_file_exists(boost::filesystem::path(
            "tests/testdata/faroe-islands-latest/roads.dbf")) == true);
  CHECK(dbf_file_exists(boost::filesystem::path(
            "tests/testdata/faroe-islands-latest/README")) == false);
  CHECK(dbf_file_exists(boost::filesystem::path(
...
TEST_CASE("ogr2wkb", "[ogr2wkb]") {
  ...
  // in postgis: select st_astext('_____wkb_reference_____'::geometry) =>
...
TEST_CASE("to_camel_case_with_spaces", "[to_camel_case_with_spaces]") {
  ...
  CHECK(to_camel_case_with_spaces("FUCHSBERGER DAMM") ==
        std::string("Fuchsberger Damm"));
  CHECK(to_camel_case_with_spaces("KÖNIGSBERGER STRASSE") ==
        std::string("Königsberger Strasse"));
