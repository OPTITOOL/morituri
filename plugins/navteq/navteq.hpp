...
link_id_type build_tag_list(ogr_feature_uptr &feat,
 ...
 */
osmium::unsigned_object_id_type build_way(ogr_feature_uptr &feat,
                                          ogr_line_string_uptr &ogr_ls,
  ...
  g_link_id_map.at(link_id).push_back(
 ...
 */
OGRLineString create_sublinestring_geometry(ogr_line_string_uptr &ogr_ls,
                                            int start_index,
                                            int end_index = -1) {
 ...
 * \param z_lvl
 */
void build_sub_way_by_index(ogr_feature_uptr &feat,
                            ogr_line_string_uptr &ogr_ls, ushort start_index,
                            ushort end_index, node_map_type *node_ref_map,
                            short z_lvl = 0) {
  ogr_line_string_uptr ogr_sub_ls_uptr(new OGRLineString(
 ...
 */
ushort
create_continuing_sub_ways(ogr_feature_uptr &feat, ogr_line_string_uptr &ogr_ls,
...
void split_way_by_z_level(ogr_feature_uptr &feat, ogr_line_string_uptr &ogr_ls,
  ...
  if (first_index != start_index) {
    build_sub_way_by_index(feat, ogr_ls, first_index, start_index,
                           node_ref_map);
    if (DEBUG)
  ...
  start_index = create_continuing_sub_ways(feat, ogr_ls, first_index,
                                           start_index, last_index, link_id,
                                           node_z_level_vector, node_ref_map);
...
void process_end_point(bool first, ushort index, z_lvl_type z_lvl,
                       ogr_line_string_uptr &ogr_ls, z_lvl_map *z_level_map,
                       node_map_type &node_ref_map) {
...
void middle_points_preparation(ogr_line_string_uptr &ogr_ls,
                               node_map_type &node_ref_map) {
 ...
 */
void set_ferry_z_lvls_to_zero(ogr_feature_uptr &feat,
...
void create_house_numbers(ogr_feature_uptr &feat, ogr_line_string_uptr &ogr_ls,
  ...
  ogr_line_string_uptr offset_ogr_ls(
...
void create_house_numbers(ogr_feature_uptr &feat,
                          ogr_line_string_uptr &ogr_ls) {
...
void build_water_poly_taglist(osmium::builder::RelationBuilder &builder,
                              ogr_layer_uptr &layer, ogr_feature_uptr &feat) {
  // Mind tl_builder scope!
  osmium::builder::TagListBuilder tl_builder(g_rel_buffer, &builder);
  tl_builder.add_tag("type", "multipolygon");
...
osm_id_vector_type build_water_ways_with_tagList(ogr_layer_uptr &layer,
                                                 ogr_feature_uptr &feat,
                                                 OGRLineString *line) {
...
osmium::unsigned_object_id_type
build_water_relation_with_tags(ogr_layer_uptr &layer, ogr_feature_uptr &feat,
...
osmium::unsigned_object_id_type
build_landuse_relation_with_tags(ogr_layer_uptr &layer, ogr_feature_uptr &feat,
                                 osm_id_vector_type ext_osm_way_ids,
                                 osm_id_vector_type int_osm_way_ids) {
  osmium::builder::RelationBuilder builder(g_rel_buffer);
  STATIC_RELATION(builder.object()).set_id(g_osm_id++);
 ...
 */
void create_multi_polygon(ogr_multi_polygon_uptr &&mp,
  ...
  for (int i = 0; i < mp->getNumGeometries(); i++) {
    ...
    osm_id_vector_type exterior_way_ids =
        build_closed_ways(poly->getExteriorRing());
...
void create_polygon(ogr_polygon_uptr &&poly,
                    osm_id_vector_type &exterior_way_ids,
...
void process_admin_boundary(ogr_layer_uptr &layer, ogr_feature_uptr &feat) {
  ...
  if (geom_type == wkbMultiPolygon) {
    create_multi_polygon(
        ogr_multi_polygon_uptr(static_cast<OGRMultiPolygon *>(geom.release())),
...
void process_water(ogr_layer_uptr &layer, ogr_feature_uptr &feat) {
  ...
  if (geom_type == wkbLineString) {
    ...
    osm_id_vector_type exterior_way_ids, interior_way_ids;
    if (geom_type == wkbMultiPolygon) {
      create_multi_polygon(ogr_multi_polygon_uptr(
                               static_cast<OGRMultiPolygon *>(geom.release())),
                           exterior_way_ids, interior_way_ids);
    } else if (geom_type == wkbPolygon) {
      create_polygon(
          ogr_polygon_uptr(static_cast<OGRPolygon *>(geom.release())),
          exterior_way_ids, interior_way_ids);
    } else {
  ...
  for (auto dir : dirs) {
    ...
    for (int i = 0; i < DBFGetRecordCount(name_place_handle); i++) {
      ...
      link_id_type link_id =
          dbf_get_uint_by_field(name_place_handle, i, LINK_ID);

      if (g_link_id_map.find(link_id) == g_link_id_map.end()) {
        std::cerr << "Skipping city node because of missing link id"
                  << std::endl;
        continue;
      }
      ...
      std::string name_type =
          dbf_get_string_by_field(name_place_handle, i, POI_NMTYPE);
      if (name_type != "B") {
      ...
      auto first_osm_id = osm_id_vector.at(0);
      const auto &first_way = g_way_buffer.get<const osmium::Way>(
          g_way_offset_map.get(first_osm_id));
      osmium::Location first_way_front = first_way.nodes().front().location();
      ...
      std::string name =
          dbf_get_string_by_field(name_place_handle, i, POI_NAME);
...
void init_g_cntry_ref_map(const boost::filesystem::path &dir,
  ...
  for (int i = 0; i < DBFGetRecordCount(cntry_ref_handle); i++) {
    ...
    auto cntry_ref = cntry_ref_type(unit_measure.c_str()[0],
                                    speed_limit_unit.c_str(), iso_code.c_str());
    g_cntry_ref_map.insert(std::make_pair(govt_code, cntry_ref));
  }
  DBFClose(cntry_ref_handle);
}

ogr_layer_uptr_vector init_street_layers(const path_vector_type &dirs,
                                         std::ostream &out) {
  ogr_layer_uptr_vector layer_vector;
  for (auto dir : dirs) {
    layer_vector.push_back(
        ogr_layer_uptr(read_shape_file(dir / STREETS_SHP, out)));
  }
  return layer_vector;
}
...
void process_way_end_nodes(ogr_layer_uptr_vector &layer_vector,
                           z_lvl_map &z_level_map) {
  for (int i = 0; i < layer_vector.size(); i++) {
    auto &layer = layer_vector.at(i);
